...
- **Hint row.** Static legend of the most-used keys and commands. In multi-pane layouts the focused outer-bottom pane shows a compact chord hint (`^W w/z/c`); other panes hide the hint. Outer-bottom panes keep a shared footer pad so column bottoms stay aligned; stacked panes above that edge use one trailing pad for uniform horizontal gutters. `chrome_compact_rows` reclaims pad only when `show_footer` is off.
