...
| `/diff` \| `review` `[N]` | Interactive review of pending patches: **[a]**pply / **[r]**eject / **[A]**llow all (session accept-edits) / Esc cancel. Streamed ```diff fences auto-enqueue review cards. Bare `/diff` walks remaining pending/failed proposals; `N` reviews one. Apply writes under the process cwd and **creates missing files**; apply is the permission grant (no `/write` confirm). |
