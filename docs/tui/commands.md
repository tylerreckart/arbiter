...
| `/diff [list]` \| `apply` \| `reject` \| `undo` `[N]` | Apply, reject, or undo a streamed ` ```diff ` patch under the process cwd. Patches register as `Patch #N` when the agent emits a fence. Omit `N` to target the latest pending (apply/reject) or applied (undo) proposal. Undo refuses if the file changed after apply. |
