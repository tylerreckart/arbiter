...
| `/diff` \| `review` `[N]` | Interactive review of pending patches: ↑↓/Enter picker (or **[a]**pply / **[r]**eject / **[A]**llow all / Esc). Streamed ```diff fences **pause the model stream** until you answer. Bare `/diff` walks remaining pending/failed proposals; `N` reviews one. Apply writes under **this conversation's workspace directory** and **creates missing files**; apply is the permission grant (no `/write` confirm). Missing/unknown roots refuse apply (no process-cwd fallback). |
