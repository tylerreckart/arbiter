...
| `/diff` \| `review` `[N]` | Interactive review of pending patches: **[a]**pply / **[r]**eject / Esc cancel. Bare `/diff` walks all pending/failed proposals; `N` reviews one. Apply writes under the process cwd and **creates missing files**; apply is the permission grant (no `/write` confirm). |
