...
| `/exec <cmd>`                  | Shell command (**on by default**). Host path always confirms with a `HOST SHELL (unsandboxed)` card. Set `ARBITER_SANDBOX_IMAGE` to opt into Docker sandbox `/exec` (same env surface as `--api`). Disable with `--no-exec`. |
| `/diff` \| `review` `[N]` | Interactive review of pending patches: **[a]**pply / **[r]**eject / **[A]**llow all (session accept-edits) / Esc cancel / PgUp·PgDn scroll. Streamed ```diff fences auto-enqueue review cards. Bare `/diff` walks remaining pending/failed proposals; `N` reviews one. Apply writes under **this conversation's workspace directory** and **creates missing files**; apply is the permission grant (no `/write` confirm). Missing/unknown roots refuse apply (no process-cwd fallback). |
...
| `/accept-edits [on\|off]` | Session flag: when on, new streamed ```diff reviews auto-apply (exec/write still confirm). Empty arg toggles. Same flag set by permission/`diff` **[A]**. |
| `/prompts` | List waiting interactive permission / diff cards and accept-edits state. |
...
| `/accept-edits [on\|off]`      | Toggle session accept-edits (auto-apply future streamed diffs). See [output-ux.md](output-ux.md). |
| `/prompts`                     | Show pending interactive permission / diff cards.                   |
