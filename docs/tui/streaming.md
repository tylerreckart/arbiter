...
Model deltas land in the scroll region as they arrive via `StreamRenderer`: `BlockParser` suppresses `/cmd` lines (unless `/verbose`) and forwards unambiguous incomplete prose immediately, `MarkdownRenderer` commits styled spans on newline and exposes a replaceable **live tail** for the in-progress line so tokens paint word-by-word, non-diff fenced code streams into `CodeSegment` (draw-time collapse — first eight body lines plus a live `… +N lines` summary, no placeholder artifacts in scrollback), and ` ```diff ` patches become `DiffSegment` panels (full-width for file adds/deletes; split for edits) with a `Patch #N` action line. Streamed patches **pause the model stream** on an interactive review card (↑↓/Enter, or `[a]`/`[r]`/`[A]`/Esc; PgUp/PgDn scrolls). Use [`/diff`](commands.md) for manual review of remaining pending patches, `/accept-edits` to keep auto-applying, or `/diff apply` to write immediately under the conversation workspace directory (creates missing files; exact hunk-header match; `/diff undo` restores when unchanged). Styling resolves from `tui.json`'s `content` tokens through `StyleId` → OpenTUI highlights — no ANSI round-trip on the master stream path. User echoes, advisor/loop/presence banners, and interrupt markers use the same prose path. Always-on `CONTEXT` notes paint as `◎ presence · <watcher>`; `SILENT` reviews stay quiet. Vertical rhythm (`block_gap` / `panel_gap` / `prose_paragraph_gap`) and panel surfaces (`code_bg`, `diff_bg_*`) are theme tokens — see [themes.md](themes.md).

Sub-agent progress uses the same pipeline with `kInterim` policy: dimmed prose, collapsed fences, capped at eight display rows / 480 columns. The master's synthesis turn carries the full result. The [fleet dashboard](fleet.md) (`Ctrl-w f`) is the live tree of those streams — depth, agent, active tool, tokens — so you do not have to read raw SSE.
