...
│   └── jules.json      Jules — pair colleague (Haiku; over-the-shoulder)
```
...
Index speaks in a **conversational** register (complete sentences, collaborative tone) while specialists keep a compressed field-report voice. Dedicated TTS agents use `mode: "spoken"` (conversation for the ear, not a compressed briefing). Routing, writ inventory, and delegation rules are shared; only the user-facing register differs. See [Voice](../concepts/voice.md).
