...
| `token_usage` | After each turn's token tally is fired off to the billing service. | `agent`, `stream_id`, `depth`, `model`, `input_tokens`, `output_tokens`, `cache_read_tokens?`, `cache_create_tokens?`. Cost figures are not included — Quartermaster's ledger is authoritative. |
...
| `error` | Recoverable errors during the request (Quartermaster denial, transient upstream issue). The stream continues or terminates depending on severity. | `message`, plus context fields (e.g. `reason`, `estimated_cost_micro_cents`, `total_budget_micro_cents` on a Quartermaster denial). |
...
## Quartermaster pre-flight denial

When Quartermaster (configured via `QUARTERMASTER_URL`) rejects the request, the runtime emits:
...
No turns run. A transport error to Quartermaster fails open — the runtime proceeds rather than blocking on a billing-service blip.
