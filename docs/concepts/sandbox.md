...
| `ARBITER_SANDBOX_WORKSPACE_MAX_BYTES`   | Per-tenant workspace disk quota, bytes. `/write` over the cap returns ERR; `/exec` snapshots then restores on over-cap or cancel. `0` = no quota. | `1073741824` (1 GiB) |
...
Writes from inside the container (e.g. `dd if=/dev/zero of=/workspace/big`) go through `docker exec`, not `/write`. When the quota is set, `/exec` copies the workspace to a sibling snapshot directory (outside the bind mount) before the command runs. If the command leaves the tree over the cap **or is cancelled**, the runtime restores that snapshot: deleted files come back, in-place overwrites revert to their original bytes, and new files are removed. Restore never follows destination symlinks — each path component is created with `openat(O_NOFOLLOW)` / `mkdirat` from the workspace directory fd, and a leftover file or symlink at a snapshotted directory name is replaced rather than walked. Incomplete snapshots skip deleting unknown paths and the error string says rollback was incomplete rather than claiming success. Cancel during the snapshot copy skips the command and does not restore.

The snapshot is bounded by the quota (a workspace already at the cap copies at most that many bytes). Operators who want a kernel-enforced ceiling can still pair this with XFS project quotas, ZFS dataset reservations, or `--storage-opt size=...` on storage drivers that support it.
...
| `ERR: workspace quota exceeded after /exec (…); used …, rolled back to …` | `/exec` left the workspace over `ARBITER_SANDBOX_WORKSPACE_MAX_BYTES`. The pre-exec snapshot was restored. | Agent adapts; workspace matches pre-exec contents. |
| `ERR: workspace quota exceeded after /exec (…); used …, rollback incomplete (now …)` | Snapshot restore could not bring usage back under the cap (permissions, incomplete inventory). | Operator inspects the tenant workspace; agent sees a failed tool. |
