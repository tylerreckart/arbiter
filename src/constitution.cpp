...
static std::string index_ai_prompt(Brevity level) {
    ...
    base +=
        ...
        "    alternatives.  Must not reuse the same agent_id twice inside one block (each\n"
        "    Agent has a single history that would race).  Grammar:\n"
        "        /parallel\n"
        "        /agent researcher_a topic A\n"
        "        /agent researcher_b topic B\n"
