...
std::string pick_master_model_default(
    ...
    if (keys.count("anthropic")) return "anthropic/claude-sonnet-5";
    if (keys.count("openai"))    return "openai/gpt-5.5";
    return {};
}
...
Orchestrator::IntentHistoryMirror::~IntentHistoryMirror() {
    if (!orch || !active || requested.empty() || dispatched.empty()) return;
    if (requested == dispatched) return;
    try { orch->mirror_agent_state(dispatched, requested); } catch (...) {}
}
    ...
    if (routed_id == "index") {
        ...
        if (ingress.applied) {
            ...
            if (requested != routed_id) {
                try {
                    mirror_agent_state(requested, routed_id);
                    hist_mirror.orch = this;
                    hist_mirror.requested = requested;
                    hist_mirror.dispatched = routed_id;
        ...
        try {
            ...
            if (ingress.applied) {
                ...
                if (requested != dispatch_id) {
                    try {
                        mirror_agent_state(requested, dispatch_id);
                        hist_mirror.orch = this;
                        hist_mirror.requested = requested;
                        hist_mirror.dispatched = dispatch_id;
                        hist_mirror.active = true;
                    } catch (...) {}
                }
            }
