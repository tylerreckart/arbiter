...
bool SseReader::feed(const char* data, size_t n) {
...
bool SseReader::process_line(const std::string& line) {
    ...
    if (name == "event") {
        if (value.size() > kSseMaxEventNameBytes) return false;
        event_name_ = value;
        have_event_ = true;
    } else if (name == "data") {
        const size_t extra = data_.empty() ? value.size() : value.size() + 1;
        if (extra > kSseMaxEventBytes ||
            data_.size() > kSseMaxEventBytes - extra) {
            return false;
        }
