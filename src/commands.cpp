...
// Thin aliases so existing call sites keep local names.  Implementation
// lives in ssrf_guard.cpp and is shared with the A2A HTTP client.
static curl_socket_t safe_opensocket_cb(void* clientp, curlsocktype purpose,
                                        struct curl_sockaddr* addr) {
    return ::arbiter::ssrf_safe_opensocket(clientp, purpose, addr);
}
static std::string preflight_ssrf_check(const std::string& url) {
    return ::arbiter::ssrf_preflight_url(url);
}
} // namespace
