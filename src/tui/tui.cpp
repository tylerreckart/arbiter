...
std::string TUI::build_welcome_card() const {
    ...
    static constexpr int kPadL = 2, kDivGapL = 2;
    static constexpr int kDivGapR = 2, kPadR = 2;
    int art_col_w  = kPadL + kArtCells + kDivGapL;
    int text_col_w = kDivGapR + text_w + kPadR;
    int inner      = art_col_w + 1 + text_col_w;
    int margin     = std::max(0, (rect_.w - (inner + 2)) / 2);
